...
export const LISTA_INTERESSES_DF = [
  ...
  { id: "concurseiro", nome: "Concurseiro" },
  ...
  unsubscribeTotalsListener = onSnapshot(totalsDocRef, (snap) => {
    ...
    sanitizedInterests.forEach(tagId => {
      ...
      if (pill) {
        ...
        const count = typeof totalsData[tagId] === "number" ? totalsData[tagId] : 0;
...
export function renderEditInterestsSelector() {
  ...
  container.innerHTML = "";
  if (countDisplay) countDisplay.textContent = `${selectedInterests.length}/5`;
    ...
    btn.addEventListener("click", (e) => {
      ...
      if (selectedInterests.includes(item.id)) {
        ...
        if (selectedInterests.length >= 5) {
          if (typeof showToast === "function") showToast("Você pode selecionar no máximo 5 tags.");
          return;
        }
